#   DrawRemoteHelpInfo.py Copyright (c) 2025 Nikki Cooper
#
#  This program and the accompanying materials are made available under the
#  terms of the GNU Lesser General Public License, version 3.0 which is available at
#  https://www.gnu.org/licenses/gpl-3.0.html#license-text
#
# Pygame IR Remote Control Help window class

import pygame
import upScale as up_scale
import remote_help_text as _remote_help_
# Define colors
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
...
class DrawRemoteHelpInfo:
    """
    The DrawRemoteHelpInfo class provides functionality to draw an IR remote control help overlay
    and manage its visibility state.

    This class is designed to render and display a help overlay on a Pygame display surface showing
    IR remote control button functions. It includes mechanisms for controlling the visibility of
    the overlay, scaling graphics based on the display resolution, and rendering styled text and
    buttons with hover effects. Additionally, it provides methods to manage and apply gradient
    rendering for smooth visual effects.
    ...
    Attributes:
        ...
        remote_help_button_rect: Pygame rectangle for the help overlay's "OK" button.
        remote_help_visible: Boolean flag indicating whether the help overlay is visible.
        is_hovered: Boolean flag indicating whether the "OK" button is currently hovered.

    Methods:
        is_visible(): Determine if the remote help overlay is currently visible.
        toggle_visibility(): Toggle the visibility state of the remote help overlay and synchronize it with the PlayVideo instance.
        set_visibility(visible): Set the visibility state of the remote help overlay and synchronize it with the PlayVideo instance.
        draw_remote_help_overlay(is_hovered): Render the remote help overlay on the display, including background, gradient, text,
                                       and button, with hover effects.
        draw_remote_help(is_hovered): Display the remote help overlay and update the hover state of the "OK" button.
    ...
    def __init__(self, play_video):
        ...
        IR remote control help content, managing scaling, fonts, icons, and other
        configurations based on the display resolution and user settings.
        ...
        Attributes:
            ...
            play_video: Instance of the PlayVideo class for synchronizing visibility.
            ...
            y_offset_text: The vertical offset used for positioning text
                relative to the display scaling.
            remote_help_button_rect: Placeholder for the rectangle area of the remote help
                button.
            remote_help_visible: A flag indicating if the remote help overlay is currently
                displayed.
    ...
    def is_visible(self):
        return self.remote_help_visible

    def toggle_visibility(self):
        self.remote_help_visible = not self.remote_help_visible
        self.play_video.remote_help_visible = self.remote_help_visible

    def set_visibility(self, visible):
        self.remote_help_visible = visible
        self.play_video.remote_help_visible = self.remote_help_visible

    def draw_remote_help_overlay(self, is_hovered):
        self.is_hovered = is_hovered
        if not self.remote_help_visible:
            return None
        ...
        DrawRemoteHelpInfo.apply_gradient(
            gradient_surface,
            (0, 0, 255),
            (0, 0, 100),
            self.BOX_WIDTH,
        ...
        y_offset = y_start
        for line in _remote_help_.REMOTE_HELP_TEXT_LEFT.split("\n"):
        ...
        y_offset = y_start
        for line in _remote_help_.REMOTE_HELP_TEXT_RIGHT.split("\n"):
            color = HEADING_COLOR if line.isupper() else TEXT_COLOR
            text_surface = (
                self.font_help_text.render(line.strip(), True, color)) \
                if not line.isupper() \
        ...
        self.remote_help_button_rect = button_rect
        return button_rect

    def draw_remote_help(self, is_hovered):
        self.is_hovered = is_hovered
        self.set_visibility(True)
        self.remote_help_button_rect = self.draw_remote_help_overlay(is_hovered)
        return self.remote_help_button_rect
